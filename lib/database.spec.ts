...
import {ConnectableObservable, noop, Observable,} from "rxjs";
import {observeOn, map, tap, mergeMapTo, publish, switchMap, skip, take} from "rxjs/operators";
...
describe('sync database in memory', () => {
    ...
    it('should update the right item', function () {
        ...
        const db = new Database<InsertType, UpdateType, RemoveType, SearchType, ItemType, false>
        (store, updatefn, undefined, undefined);
    ...
    it('should update if there is a match otherwise insert in a list of items', function () {
        ...
        expect(db.dbCore.storeBase.state.container.find(x => x.name === 'item2')).is.equal(res[0]);
        expect(db.dbCore.storeBase.state.container.find(x => x.name === 'item4')).is.equal(res[1]);
        expect(db.dbCore.storeBase.state.container.length).is.equal(4);
    });
});